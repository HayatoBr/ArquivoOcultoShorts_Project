...
echo [ArquivoOculto][SDXL][PROD] Python da venv:
...
echo [ArquivoOculto][SDXL][PROD] Iniciando pipeline...

"%ROOT%\.venv\Scripts\python.exe" "%ROOT%\main.py" --config "%ROOT%\config.sdxl.yml"

echo [ArquivoOculto][SDXL][PROD] Encerrado com codigo %ERRORLEVEL%.
pause